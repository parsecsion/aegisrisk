...
class PrologBridge:
    ...
    def evaluate(
        ...
        except Exception as exc:  # bare except is bad but pyswip throws weird errors
            return Decision(
                status="reject",
    ...
    def _make_prolog_safe_string(self, value: str) -> str:
        """
    ...
    def _assert_ticket(self, ticket: TradeTicket) -> None:
        symbol_atom = self._make_prolog_safe_string(ticket.symbol)
        side_atom = self._make_prolog_safe_string(ticket.side.value)
        order_type_atom = self._make_prolog_safe_string(ticket.order_type.value)
        tif_atom = self._make_prolog_safe_string(ticket.time_in_force.value)
    ...
    def _assert_market_and_state(self, market: MarketState, fuzzy_facts: Dict[str, str]) -> None:
        symbol_atom = self._make_prolog_safe_string(market.symbol)
        vol_level = self._make_prolog_safe_string(fuzzy_facts["volatility_level"])
        spread_level = self._make_prolog_safe_string(fuzzy_facts["spread_level"])
        exposure_level = self._make_prolog_safe_string(fuzzy_facts["exposure_level"])
        sentiment_level = self._make_prolog_safe_string(fuzzy_facts["sentiment_level"])
        session_atom = self._make_prolog_safe_string(fuzzy_facts["session"])
    ...
    def _assert_account_tier(self, account_tier: str) -> None:
        ...
        tier_atom = self._make_prolog_safe_string(account_tier)
